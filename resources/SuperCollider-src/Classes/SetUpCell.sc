SetUpCell{
	...
	*new{arg aServer;
	...
	loadSynthDefs{
		...
		SynthDef(\sample, { | pan=0, amp=1.0, bufnum = 0, outbus = 0, midi = 0, loop = 0 |
			var sample;
		...
		//Sine Beep
		SynthDef(\beep,{ | freq=440,amp=0.1, pan=0.0, dur=0.1, attack = 0.5, outbus = 16 |
			var env = EnvGen.kr(Env.perc(attack,dur),doneAction:2);
			OffsetOut.ar(outbus,Pan2.ar(SinOsc.ar(freq,0,amp)*env,pan))
		}).add;

		//Pulse Beep
		SynthDef(\pulse,{ | freq=440,amp=0.1, pan=0.0, dur=0.1, attack = 0.5, outbus = 16 |
			var env = EnvGen.kr(Env.perc(attack,dur),doneAction:2);
			OffsetOut.ar(outbus,Pan2.ar(Pulse.ar(freq,0.5,amp)*env,pan))
		}).add;

		//Delay
		SynthDef(\delay, { | inbus = 16, outbus = 18, mix=0.5, time=0.375, feedMix=0.375, amp=1.0 |
			...
			LocalOut.ar(local);
			Out.ar(outbus, local * amp);
		}).add;

		//Reverb FX
		SynthDef(\reverb, {| inbus = 18, outbus = 20, mix = 0.4, room = 0.8, damp = 0.7, amp = 0.9, length = 1 |
		...
		SynthDef(\compressor, { |inbus = 22 |
			var signal, output;
			signal = In.ar(inbus, 2);
			signal = BPeakEQ.ar(signal, 1000, 0.5, -12);
			output = Limiter.ar(signal, 1.0, 0.01);
			Out.ar(0,output*1.0);
		}).add;
